...
class TestDynamicCandidateSamplingLoss(unittest.TestCase):
  ...
  @unittest.skip("TODO: implement dynamic n_element hit test.")
  def test_compute_accidental_hit_dynamic_element(self):
    pass

  def test_compute_sampled_logits_single_element(self):  # sess = tf.Session()
    with tf.Session() as sess:
      dSampledLoss = DynamicSampledSoftmaxLoss("./cmake-build-debug/libdynamic_sampled_softmax_loss.dylib")
      true_classes = tf.Variable([1,
                                  2,
      ...
      softmax_weights = tf.get_variable("w2", [50, 36], dtype=tf.float32)
      softmax_biases = tf.get_variable("b2", [50], dtype=tf.float32)
      inputs = tf.get_variable("inputs2", [7, 36], dtype=tf.float32)

      tf_logits, tf_labels, tf_true_expected_count, tf_sampled_expected_count = dSampledLoss._tf_compute_sampled_logits_helper(
        softmax_weights,
        softmax_biases,
        ...
        num_sampled=13,
        num_classes=50,
        sampled_values=tf.nn.uniform_candidate_sampler(true_classes=tf.expand_dims(true_classes, 1),
                                                       num_true=1,
                                                       num_sampled=13,
                                                       unique=True,
                                                       range_max=50,
      ...
      tf_true_logits = tf.slice(tf_logits, [0, 0], [-1, 1])
      tf_sampled_logits = tf.slice(tf_logits, [0, 1], [-1, -1])

      true_logits, sampled_logits, true_expected_count, sampled_expected_count = dSampledLoss._compute_sampled_logits_helper(
        softmax_weights,
        softmax_biases,
        labels=true_classes,
        num_true=num_true,
        ...
        num_classes=50,
        num_sampled=13,
        sampled_values=dSampledLoss._uniform_candidate_sampler(true_classes, num_true,
                                                               num_sampled=13, unique=True,
                                                               range_max=50, seed=12))
      ...
      tf_true_logits_res = sess.run(tf_true_logits)
      true_logits_res = sess.run(true_logits)
      tf_true_expected_count_res, tf_sampled_expected_count_res, \
      true_expected_count_res, sampled_expected_count_res = sess.run([tf_true_expected_count,
                                                                      tf_sampled_expected_count,
                                                                      true_expected_count,
                                                                      sampled_expected_count])

      np.testing.assert_almost_equal(tf_true_expected_count_res.flatten(), true_expected_count_res)
      np.testing.assert_almost_equal(tf_sampled_expected_count_res, sampled_expected_count_res)

      np.testing.assert_almost_equal(true_logits_res, tf_true_logits_res.flatten())

      np.testing.assert_almost_equal((tf_true_logits_res - np.log(tf_true_expected_count_res)).flatten(),
                                     (true_logits_res - np.log(true_expected_count_res)))

      tf_sampled_logits_res = sess.run(tf_sampled_logits)
      sampled_logits_res = sess.run(sampled_logits)

      np.testing.assert_almost_equal(sampled_logits_res, tf_sampled_logits_res)

  def test_compute_sampled_logits_multiple_element(self):  # sess = tf.Session()
    with tf.Session() as sess:
      dSampledLoss = DynamicSampledSoftmaxLoss("./cmake-build-debug/libdynamic_sampled_softmax_loss.dylib")
      true_classes = tf.Variable([1, 11,
                                  2, 12,
                                  ...
                                  5, 15,
                                  6, 16,
                                  7, 17], trainable=False, dtype=tf.int64)
      ...
      softmax_weights = tf.get_variable("w", [50, 36], dtype=tf.float32)
      softmax_biases = tf.get_variable("b", [50], dtype=tf.float32)
      inputs = tf.get_variable("inputs", [7, 36], dtype=tf.float32)

      tf_logits, tf_labels, tf_true_expected_count, tf_sampled_expected_count = dSampledLoss._tf_compute_sampled_logits_helper(
        softmax_weights,
        softmax_biases,
        labels=tf.reshape(true_classes, [-1, 2]),
        inputs=inputs,
        num_sampled=13,
        num_classes=50,
        sampled_values=tf.nn.uniform_candidate_sampler(true_classes=tf.reshape(true_classes, [-1, 2]),
                                                       num_true=2,
                                                       num_sampled=13,
                                                       unique=True,
                                                       range_max=50,
      ...
      tf_true_logits = tf.slice(tf_logits, [0, 0], [-1, 2])
      tf_sampled_logits = tf.slice(tf_logits, [0, 2], [-1, -1])
  ...
  def test_compute_gradient_single_element(self):
    with tf.Session() as sess:
      ...
      true_classes = tf.Variable([1], trainable=False, dtype=tf.int64)

      num_true = tf.Variable([1], trainable=False, dtype=tf.int64)
      ...
      inputs = tf.get_variable("inputs_grad_single", [1, 5], dtype=tf.float32)
